...
class WhatNextViewSpec extends CommonPlaySpec with WithCommonFakeApplication with MockitoSugar with FakeRequestHelper {
  ...
  "What next view should" when {
    ...
    lazy val view = whatNextView(isDateAfter = false, 30)(using FakeRequest("GET", ""), mockMessage)
    ...
    "should produce the same output when render and f are called" in {
      whatNextView(false, 30)(using FakeRequest("GET", ""), mockMessage) shouldBe whatNextView.render(false, 30, FakeRequest("GET", ""), mockMessage)
    }
  }

  "Disposal date is after 6 April 2020" which {
    lazy val view2 = whatNextView(isDateAfter = true, 30)(using FakeRequest("GET", ""), mockMessage)
